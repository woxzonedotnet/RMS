...
namespace RMS.Forms.Inventory
{
    public partial class frmSupplierReturnNote : Form
    {
        ...
        private string SelectionFormularValues(int iReportID)
        {
            ...
            {
                srtFormular += "{" + oReportMaster.SelectedTable + ".fldSRNCode}='" + this.txtSRNNumber.Text + "'"; // +"' AND {tbl_daily_in_out_details.fldAttendanceDate}=#" + Convert.ToDateTime(dFromDate).ToString("yyyy-MM-dd") + "# TO #" + Convert.ToDateTime(dToDate).ToString("yyyy-MM-dd") + "# ";
            }
